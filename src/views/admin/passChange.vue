      ...
      <b-card class="mb-4">
        ...
        <b-form @submit.prevent="onValitadeFormSubmit" class="av-tooltip tooltip-label-right">
          ...
          <div class="button-row">
            ...
            <b-button type="submit" style="color:#fff;" variant="primary" size="lg" class="mt-4">비밀번호 변경</b-button>
          </div>
        </b-form>
...
<script>
  import {API_URL_ADMIN_CHAGEPASSWORD, CONFIG_HEADER_POST} from '../../api/urls';
  ...
  export default {
    ...
    validations: {
      ...
      newpass: {
        required,
        async checkNewpass(pw) {
          if (this.$v.newpass.required) {
            if (pw.length < 8 || pw.length > 15) return false;
            let pattern = /([a-zA-Z0-9].*[!,@,#,$,%,^,&,*,?,_,~,-])|([!,@,#,$,%,^,&,*,?,_,~,-].*[a-zA-Z0-9])/;
            return (pattern.test(pw)) ? true : false;
          }
        },
      ...
      renewpass: {
        required,
        async checkEqualOrg(pw) {
          if (this.$v.renewpass.required && this.$v.newpass.$model) {
            if (pw == this.$v.newpass.$model) return true;
            else return false;
          }
        }
      }
    ...
    methods: {
      ...
      updatePassword() {
        ...
        this.$http.post(API_URL_ADMIN_CHAGEPASSWORD, params, {headers: CONFIG_HEADER_POST}).then(res => {
