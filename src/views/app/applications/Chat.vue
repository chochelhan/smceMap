<template>
  <div>
    <b-row class="app-row">
      <b-colxx xxs="12" class="chat-app">
        <conversation-detail v-if="isLoadContacts && isLoadConversations && otherUser!=null" key="conversation"
                             :current-user="currentUser" :other-user="otherUser" :messages="conversationMessages"/>
    ...
    <div class="chat-input-container d-flex justify-content-between align-items-center">
      <b-input class="flex-grow-1" type="text" :placeholder="$t('chat.saysomething')" v-model="message"
    ...
    <application-menu>
      <b-tabs no-fade class="pl-0 pr-0 h-100" content-class="chat-app-tab-content"
              nav-class="card-header-tabs ml-0 mr-0" v-model="tabIndex">
        <b-tab :title="$t('chat.messages')" active title-item-class="w-50 text-center" no-body
          ...
          <conversation-list v-if="isLoadContacts && isLoadConversations" key="conversationList"
                             :current-user="currentUser" :conversations="conversations" :contacts="contacts"
