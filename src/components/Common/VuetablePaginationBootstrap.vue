  ...
  <nav>
    <ul class="pagination justify-content-center pagination-sm">
      ...
      <template v-if="notEnoughPages">
        <li v-if="notEnoughPages" v-for="n in totalPage" :class="{'active': isCurrentPage(n), 'page-item':true}"
            :key="n">
      ...
      <template v-else>
        <li v-for="n in windowSize" :class="{'active': isCurrentPage(windowStart+n-1), 'page-item':true}" :key="n">
          <a class="page-link" href="javascript:" @click.prevent="loadPage(windowStart+n-1)"
             v-html="windowStart+n-1"></a>
        </li>
      </template>
